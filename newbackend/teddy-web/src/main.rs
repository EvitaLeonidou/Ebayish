use teddy_infrastructure::{get_configuration, get_subscriber, init_subscriber};
use teddy_services::admin::seeding::SeedingService;
use teddy_services::auctions::management::AuctionService;
use teddy_web::startup::Application;
use teddy_domain::utils::password::hash_password;
use secrecy::Secret;
...
async fn main() -> std::io::Result<()> {
    ...
    let connection_pool = teddy_web::startup::get_connection_pool(&configuration.database);
    ...
    if configuration.seeding.should_run_seeding() {
        ...
        while seeding_attempts < max_attempts {
            ...
            match SeedingService::seed_database(&connection_pool, &configuration.seeding).await {
                Ok(()) => {
                    tracing::info!("Database seeding completed successfully");
                    break;
                }
                Err(teddy_services::admin::seeding::SeedingError::AlreadyCompleted) => {
